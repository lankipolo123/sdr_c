...
static void build_controls(HWND hwnd) {
    ...
    /* Sidebar: one tall box, Activity Log fills it top to bottom - no
     * reserved empty space. Connection & Settings and Amplifier
     * Temperature moved up into the header above. */
    ...
    add_ctrl(hwnd, "LISTBOX", NULL, LBS_NOTIFY | LBS_NOINTEGRALHEIGHT | WS_VSCROLL | WS_TABSTOP | WS_BORDER,
             22, CONTENT_TOP + 30, SIDEBAR_W - 44, GRID_ROWS * CARD_H + (GRID_ROWS - 1) * CARD_GAP - 38, IDC_LOG_LISTBOX);
...
static void relayout_for_size(HWND hwnd, int client_w, int client_h) {
    ...
    MoveWindow(GetDlgItem(hwnd, IDC_LOG_LISTBOX), 22, CONTENT_TOP + 30, SIDEBAR_W - 44, GRID_ROWS * CARD_H + (GRID_ROWS - 1) * CARD_GAP - 38, FALSE);
